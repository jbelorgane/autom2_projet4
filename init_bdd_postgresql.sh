...
sudo -u postgres ${BDD_CMD} -c "\c ${DB_NAME}"
retour=$?
if [ $retour -eq 0 ] 
then
    ...
    sudo -u postgres ${BDD_CMD} -f ./${BDD_INIT_SQL}
    echo " effectuée." 
    
    # Alimentation de la BDD
    echo "\nAlimentation de la base ${BDD_ENGINE} '${DB_NAME}'...\c"
    sudo -u postgres ${BDD_CMD} ${DB_NAME} -f ./${BDD_INIT_DUMP}
    echo " effectuée."
fi
